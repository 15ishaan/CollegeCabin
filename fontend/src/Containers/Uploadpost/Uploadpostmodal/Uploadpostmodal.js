...
export default function FormDialog(props) {
  ...
  const submit = () => {
    ...
    Axios.post("http://00409ed8638e.ngrok.io/uploadPost/file/" + username +'/'+ filetype, fd,{
    ...
    //  this.props.history.push("/newsfeed/" + this.props.match.params.userename)
    })
    .catch((error) =>{
        console.log(error);
    })
    ...
    <div>
      ...
      >
        ...
        <DialogContent>
          ...
          <label  style={{ cursor: filetype!='null'?'no-drop':'pointer'}}  htmlFor='image'>
            <div>
              <img
                height="25px"
                width="25px"
            ...
            <input type='file' onChange={onChangeHandler} style={{display:'none'}} name='file' accept='.jpeg,.png,jpg' id='image' ></input>

            
            <label style={{ cursor: filetype != 'null'?'no-drop':'pointer'}} htmlFor='document'><div> 
            ...
            <input style={{display:'none'}}  accept=".pdf,.doc"  name='file' onChange={onChangeHandler} type='file' id='document'></input>
          </div>
          <div>
